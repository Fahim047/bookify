...
export const addHotelRoom = async (roomFormData: FormData) => {
...
export const fetchRoomsByHotelId = async (
	...
	const response = await fetch(
		`${API_BASE_URL}/api/rooms/getRoomsByHotelId/${hotelId}`
	);
	if (!response.ok) {
...
export const fetchBookingsByHotelId = async (
	hotelId: string
): Promise<BookingWithHotelType[]> => {
